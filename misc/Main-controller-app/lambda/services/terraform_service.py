...
class TerraformService:
    ...
    def _check_config(self):
        if not self.api_token or not self.org_name:
            raise ValueError("TFC_API_TOKEN and TFC_ORG_NAME environment variables required")
    
    def get_workspace_status(self, project: str) -> str:
        try:
            self._check_config()
            workspace_name = f"portfolio-{project}"
    ...
    def trigger_apply(self, project: str) -> None:
        self._check_config()
        workspace_name = f"portfolio-{project}"
        ...
        payload = {
            "data": {
                ...
                "relationships": {
                    "workspace": {
                        "data": {"type": "workspaces", "id": workspace_name}
                    }
                }
            }
        }
    ...
    def trigger_destroy(self, project: str) -> None:
        self._check_config()
        workspace_name = f"portfolio-{project}"
        url = f"{self.base_url}/runs"
